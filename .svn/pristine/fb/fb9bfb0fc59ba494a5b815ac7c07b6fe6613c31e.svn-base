...
document.addEventListener("DOMContentLoaded", () => {
	   ...
	   // 부서 추가 모달 열기
		  ...
		  function loadEmployeeListForManager() {
		      $.ajax({
		          type: "GET",
		          url: `${contextPath}/${companyNo}/organization/admin/employees`, // 🔥 사원 목록 조회용 URL
		          success: function (employees) {
		  ...
		  $('#add-dept-btn, #editDeptBtn').on('click', function () {
		      $('#addDeptModal').fadeIn();
		      loadEmployeeListForManager(); // 🔥 사원 리스트 불러오기
		  });
		  ...
		  $('#addDeptForm').on('submit', function (e) {
		      ...
		      $.ajax({
		          ...
		          error: function () {
		              // ❌ 실패 시: 모달은 유지, Swal만 띄우기 (앞에 띄우는 거라 z-index 문제 없음)
		              Swal.fire({
						  toast: true,
					   	  position: 'top',
		                  icon: 'error',
